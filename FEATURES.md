...
| ultiessentials.home.set-named.outcome-verified | Re-running `/sethome <name>` on an existing home re-queries the stored record afterwards and compares its coordinates with the sender's position, reporting a distinct failure when the move did not reach the store instead of confirming a move that would still teleport the player to the old location | command | `/sethome <existing name>` | ultiessentials.sethome | player | player | brief | HomeService#setHome |
