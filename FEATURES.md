...
| ultiessentials.config.essentials.features.scheduled-commands.commands | The scheduled command list, each entry `interval_seconds:command`; a malformed entry (missing colon, non-numeric or non-positive interval, empty command) is logged and skipped, not rejected as a whole-file validation failure. The Java default is the two entries every earlier version shipped; only the human-readable words after `say`/`broadcast` are written in the server's language at module enable and reload (`ultiessentials.lifecycle.config-text-materializer`) -- the interval prefix, the command verb and the colour code never change. A scheduled task re-reads the file at every run, so a value the module rewrites after the task was scheduled is dispatched from its next run | config | `config/essentials.yml: features.scheduled-commands.commands (default: the shipped 2 entries, in the server's language)` | n/a | n/a | admin | detailed | ScheduledCommandService#startTasks |
