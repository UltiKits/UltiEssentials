...
| ultiessentials.chestlock.protect-piston-extend | Cancel a piston-extend event outright if any block it would move belongs to a locked container (container-scoped for consistency; vanilla will not let a piston move a tile-entity container at all, so no exploitable route through this handler is known) | event | extend a piston toward a locked container | n/a | n/a | internal | none | ChestLockListener#onPistonExtend |
| ultiessentials.chestlock.protect-piston-retract | Cancel a piston-retract event outright if any block it would move belongs to a locked container, container-scoped as for piston-extend | event | retract a sticky piston pulling a locked container | n/a | n/a | internal | none | ChestLockListener#onPistonRetract |
