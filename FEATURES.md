...
- **Kind**, exactly these eight values: `command`, `config`, `event`, `gui`, `scheduled`,
  ...
  This module has no `gui` rows (no GUI page class — see the `## Configuration` section note
  below) and no `placeholder`
...
Source of exactly one `command`-Kind row below, checked by identity, not merely by count — a table that
reports 38 (classes) instead of 57 (formats) is wrong. There are 58 `command` rows: the 58th,
`ultiessentials.home.set-named.outcome-verified`,
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultiessentials.ban.ban | Permanently ban a player (online or previously-seen offline) with the default reason `No reason` when none is given (in the server's language at the time of the ban, and stored with the ban), kicking them immediately if online, and announcing the ban — to the whole server, or to the issuer alone when `features.ban.broadcast-ban` is `false` | command | `/ban <player>` (alias `/eban`) | ultiessentials.ban | both | admin | brief | BanCommand#ban |
...
| ultiessentials.ban.tempban | Temporarily ban a player for a parsed duration (`1d`/`2h`/`30m`/`1w`, combinable e.g. `1d12h30m`) with the same default reason as `ultiessentials.ban.ban` (see that row), refusing on an unparseable duration, and announcing the ban as `ultiessentials.ban.ban` does | command | `/tempban <player> <duration>` | ultiessentials.ban.temp | both | admin | brief | TempBanCommand#tempban |
...
deliberately in-memory only. **This module excludes no GUI class from the coverage gate** (it ships
no GUI page) — so every row's `Covers` column throughout this document is left blank.
...
(`UltiEssentialsLanguageCatalogueTest`, `UltiEssentialsCjkLiteralScopeTest`) now fail the build on a
missing key or on Chinese text outside a catalogue. Exception messages thrown to code stay English.
...
| ultiessentials.config.essentials.features.scoreboard.title | The sidebar scoreboard's title line, PlaceholderAPI-or-fallback-substituted and color-coded; the Java default is the title every earlier version shipped, written in the server's language at module enable and reload (`ultiessentials.lifecycle.config-text-materializer`); `@NotEmpty` refuses a blank value at load | config | `config/essentials.yml: features.scoreboard.title (default: the shipped title, in the server's language)` | n/a | n/a | admin | none | ScoreboardService#updateScoreboard |
| ultiessentials.config.essentials.features.scoreboard.lines | The sidebar scoreboard's body lines, in display order (the Java default is the ten-line list every earlier version shipped, written in the server's language at module enable and reload -- `ultiessentials.lifecycle.config-text-materializer`; an empty list is the operator's way to show no lines and is never materialized), each PlaceholderAPI-or-fallback-substituted and color-coded; a duplicate rendered line under 40 characters is disambiguated with an appended invisible `ChatColor`, but `ScoreboardService#ensureUnique` checks the untruncated candidate against already-added (and therefore already-truncated) entries before truncating its own result at the end -- for two lines that are identical only in their first 40+ characters, the collision check never fires, so the second line is truncated to the SAME 40-character string as the first and overwrites it as one `Score` entry rather than appearing as a second line | config | `config/essentials.yml: features.scoreboard.lines (default: the shipped ten-line list, in the server's language)` | n/a | n/a | admin | detailed | ScoreboardService#updateScoreboard, ScoreboardService#ensureUnique |
...
| ultiessentials.config.essentials.features.scheduled-commands.commands | The scheduled command list, each entry `interval_seconds:command`; a malformed entry (missing colon, non-numeric or non-positive interval, empty command) is logged and skipped, not rejected as a whole-file validation failure. The Java default is the two entries every earlier version shipped; while the list is still that built-in text it is written in the server's language at module enable and reload (`ultiessentials.lifecycle.config-text-materializer`), and the languages differ only in the human-readable words after `say`/`broadcast` -- the interval prefix, the command verb and the colour code are the same. Each task sends the command it was scheduled with: the tasks are restarted at module enable when the list was just rewritten, and at every reload, so an edit of the list takes effect at the next `/ul reload` | config | `config/essentials.yml: features.scheduled-commands.commands (default: the shipped 2 entries, in the server's language)` | n/a | n/a | admin | detailed | ScheduledCommandService#startTasks |
...
| ultiessentials.config.essentials.features.deathpunish.command.commands | Console commands run on a punished death, with `{PLAYER}`/`%player%` substitution. The Java default is the entry every earlier version shipped; only the human-readable word after `say` is written in the server's language at module enable and reload (`ultiessentials.lifecycle.config-text-materializer`) -- `say` and `{PLAYER}` never change | config | `config/essentials.yml: features.deathpunish.command.commands (default: the shipped entry, in the server's language)` | n/a | n/a | admin | none | DeathPunishListener#executeCommands |
...
| ultiessentials.config.motd.motd.line1 | MOTD first line, `&`-color-coded; the Java default is the line every earlier version shipped, written in the server's language at module enable and reload (`ultiessentials.lifecycle.config-text-materializer`) | config | `config/motd.yml: motd.line1 (default: "&6Welcome to our server!", in the server's language)` | n/a | n/a | admin | brief | MotdListener#onServerListPing |
| ultiessentials.config.motd.motd.line2 | MOTD second line, `&`-color-coded; the Java default is the line every earlier version shipped, written in the server's language at module enable and reload (`ultiessentials.lifecycle.config-text-materializer`) | config | `config/motd.yml: motd.line2 (default: "&7Powered by UltiTools", in the server's language)` | n/a | n/a | admin | brief | MotdListener#onServerListPing |
...
| ultiessentials.config.tabbar.tabbar.header | Tab-list header template, `&`-color-coded, with `%online%`/`%max%` substitution; the Java default is the header every earlier version shipped, written in the server's language at module enable and reload (`ultiessentials.lifecycle.config-text-materializer`); neither `tabbar.header` nor `tabbar.footer` carries `@NotEmpty`, and a blank value is the operator's way to show nothing and is never materialized | config | `config/tabbar.yml: tabbar.header (default: the shipped header, in the server's language)` | n/a | n/a | admin | brief | TabBarListener#updateTabBar |
...
| ultiessentials.config.tabbar.tabbar.footer | Tab-list footer template, `&`-color-coded, with `%online%`/`%max%` substitution; the Java default is the footer every earlier version shipped, written in the server's language at module enable and reload (`ultiessentials.lifecycle.config-text-materializer`); see `ultiessentials.config.tabbar.tabbar.header` for the blank semantics | config | `config/tabbar.yml: tabbar.footer (default: the shipped footer, in the server's language)` | n/a | n/a | admin | brief | TabBarListener#updateTabBar |

## Language

Every chat, kick-screen and console line follows the framework-wide `language` setting; see the Language note under `## Data Persistence`.
...
| ultiessentials.lifecycle.config-text-materializer | On module enable and again on every reload of it (`UltiEssentials#registerSelf`, `#onReload`, both after the framework has read `config/essentials.yml`, `config/tabbar.yml` and `config/motd.yml` and loaded the language -- never from a configuration change listener, which the framework fires before it reloads the language): the scoreboard title and lines, the scheduled-command text, the death-punishment command text, the tab-list header and footer, and the two MOTD lines are each checked, and one that is still built-in text -- the default an earlier version shipped, or this jar's English or Chinese text for it (read from the module jar, never from the language files on disk) -- and differs from the current text is replaced with this jar's built-in text for the server's language; each of the three files that changed is saved once, and when `config/essentials.yml` changed at module enable the scheduled-command tasks are restarted so they send the written text. An untouched value therefore follows a `language` switch in both directions; a value that differs in any way, even by one character, is the operator's and is kept byte for byte; a text that would break `features.scoreboard.title`'s `@NotEmpty` is never written; a second start with the same language writes nothing. A single-module `/ul reload UltiEssentials` does not re-read the framework's `language`, so a changed `language` is picked up on a bare `/ul reload` or a restart. A failed save is logged as one warning naming the file (`essentials.log.config_default_save_failed`). The text written is this jar's own built-in text for the server's language (the jar's `lang/<language>.json`), not the extracted language file on disk, so every value the module writes is one it recognises again; these settings are edited in the config files, and an edit of the extracted language file does not change them (earlier versions never read them from the language file either; the text source decision of 2026-09-25) | event | automatic, at module enable and at `/ul reload` | n/a | n/a | admin | brief | UltiEssentials#registerSelf, UltiEssentials#onReload, EssentialsConfig#materializeText, TabBarConfig#materializeText, MotdConfig#materializeText, ConfigTextDefaults |
