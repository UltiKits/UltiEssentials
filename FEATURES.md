...
| ultiessentials.lock.protect-whole-container | A lock record on either half of a double chest protects the shared inventory behind both, because the interact check is keyed on the container rather than on the block the player clicked; a stranger is refused on both halves even when only one half holds a record | event | click either half of a double chest only one half of which is recorded as locked | n/a | n/a | internal | none | ChestLockService#canAccess(Block, Player), ChestLockListener#onPlayerInteract |
