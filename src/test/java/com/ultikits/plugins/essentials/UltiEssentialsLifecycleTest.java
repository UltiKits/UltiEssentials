 ...
 * they carried no work of their own. The module declares two hooks: {@code onReload()}, which
 * restarts the scheduled-command, scoreboard and name-prefix services against the re-read
 * configuration (UltiKits/UltiEssentials#28), and {@code onUnregister()}, which stops every
 * repeating task it started so unloading the module really stops them
 * (UltiKits/UltiEssentials#43).
 * <p>
 * 生命周期结构约束：本模块不再声明框架的模板方法，只声明 onReload() 与 onUnregister() 两个钩子（#28、#43）。
 */
@DisplayName("UltiEssentials lifecycle contract (UltiKits/UltiEssentials#23, #28, #43)")
class UltiEssentialsLifecycleTest {

    @Test
    @DisplayName("declares no framework template method and both the onReload() and onUnregister() hooks")
    void declaresNoTemplateMethodAndBothHooks() {
