package com.ultikits.plugins.essentials.i18n;

import com.ultikits.plugins.essentials.i18n.UltiEssentialsLanguageCatalogueTest.Catalogue;
 ...
 * ({@link UltiEssentialsLanguageCatalogueTest#loadModuleCatalogues()}: the framework's own {@link Language},
...
public final class CatalogueText {
    ...
    public static Map<String, String> entries(String code) {
        try {
            for (Catalogue c : UltiEssentialsLanguageCatalogueTest.loadModuleCatalogues()) {
    ...
    /** A mock module whose {@code i18n} answers from {@code code}'s catalogue, for services that look text up. */
    public static com.ultikits.ultitools.abstracts.UltiToolsPlugin plugin(String code) {
        ...
        org.mockito.Mockito.lenient().when(plugin.i18n(org.mockito.ArgumentMatchers.anyString())).thenAnswer(answer(code));
        return plugin;
    }
